...
int main(int argc, char const* argv[]) {
  ...
  using car_parking_profile = car_parking<false>;
  using generic_car_parking_profile = combi_profile<std::tuple<car, foot<false>>, std::tuple<transitions::is_parking>>;

  static_assert(sizeof(car_parking_profile::node) == 8);
  static_assert(sizeof(generic_car_parking_profile::node) == 12);
  static_assert(sizeof(car_parking_profile::label) == 12);
  static_assert(sizeof(generic_car_parking_profile::label) == 16);
  static_assert(sizeof(car_parking_profile::entry) == 288);
  static_assert(sizeof(generic_car_parking_profile::entry) == 624);
  static_assert(sizeof(generic_car_parking_profile::entry::pred_) == 384);

  fs::path result_path = "./results/";
  fs::create_directories(result_path);

  fs::path car_result_path = result_path / "car_parking.csv";
  fs::path generic_result_path = result_path / "car_parking_generic.csv";


  runBenchmark<car_parking_profile>(car_result_path, opt.threads_, opt.n_queries_, opt.max_dist_, w);
  runBenchmark<generic_car_parking_profile>(generic_result_path, opt.threads_, opt.n_queries_, opt.max_dist_, w);
}
